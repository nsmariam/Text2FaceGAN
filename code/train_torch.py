...
# ── Hyperparamètres (valeurs du papier) ──────────────────────────
Z_DIM                 = 100
T_DIM                 = 256
BATCH_SIZE            = 64
IMAGE_SIZE            = 64
...
def main():
    ...
    netD = Discriminator(T_DIM, DF_DIM, CAPTION_VECTOR_LENGTH).to(DEVICE)
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_no, (real_imgs, wrong_imgs, captions) in enumerate(
            ...
            # ── Generator update x2 (comme l'auteur) ─────────────
            for _ in range(2):
                optimG.zero_grad()
            ...
            # Label flipping toutes les 4 itérations (technique du papier)
            if batch_no % 4 == 0:
        ...
        # Checkpoint toutes les 5 époques
        if epoch % 5 == 0:
            torch.save({
                'epoch'  : epoch,
            ...
            }, os.path.join('/content/drive/MyDrive/text2facegan_checkpoints',
